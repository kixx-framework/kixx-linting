...
const indentRule = {
    ...
    create(context) {
         ...
         */
        function processStatements(statements, level, parentNode) {
            if (!statements || statements.length === 0) return;
            for (const stmt of statements) {
                if (stmt) {
                    setExpected(stmt.loc.start.line, level);
                    processNode(stmt, level, parentNode);
                }
            }
        }
         ...
         */
        function processNode(node, level, parentNode) {
            ...
            switch (node.type) {
                ...
                case "ArrowFunctionExpression": {
                    if (node.body && node.body.type === "BlockStatement") {
                        ...
                        processStatements(node.body.body, level + 1, node.body);
                    }
                    break;
                }

                case "BlockStatement": {
                    const container = parentNode ?? node.parent;
                    const isInlineSwitchCaseBlock = container?.type === "SwitchCase" &&
                        node.loc.start.line === container.loc.start.line;

                    if (node.loc.start.line !== node.loc.end.line) {
                        setExpected(node.loc.end.line, isInlineSwitchCaseBlock ? level - 1 : level);
                    }
                    processStatements(node.body, isInlineSwitchCaseBlock ? level : level + 1, node);
                    break;
                }
                    ...
                    if (node.alternate) {
                        ...
                        if (node.alternate.type === "IfStatement") {
                            ...
                            if (node.alternate.type === "BlockStatement") {
                                ...
                                processStatements(node.alternate.body, level + 1, node.alternate);
                            } else {
                ...
                case "SwitchStatement": {
                    ...
                    for (const caseNode of node.cases) {
                        ...
                        processStatements(caseNode.consequent, level + switchCaseIndent + 1, caseNode);
                    }
                    break;
                }
                ...
                case "ClassExpression": {
                    if (node.body) {
                        ...
                        for (const member of node.body.body) {
                            ...
                            processNode(member, level + 1, node.body);
                        }
                    }
                    break;
                }
                ...
                case "ExportDefaultDeclaration": {
                    if (node.declaration) {
                        ...
                        processNode(node.declaration, level, node);
                    }
                    break;
                }
                ...
                case "Program": {
                    processStatements(node.body, 0, node);
                    break;
                }
