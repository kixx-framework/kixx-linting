...
const indentRule = {
    ...
    create(context) {
        ...
        const indentSize = rawIndent === "tab" ? 1 : (typeof rawIndent === "number" ? rawIndent : 4);
        ...
        /**
         * Get the indent level (in units) from a line's leading whitespace.
         */
        function getActualIndentLevel(line) {
            if (indentType === "tab") {
                let count = 0;
        ...
        /**
         * Get the raw leading whitespace of a line.
         */
        function getActualIndentStr(line) {
            if (indentType === "tab") {
        ...
        return {
            ...
            "Program:exit"() {
                ...
                // Build a map from line number to the first token on that line
                ...
                for (const [lineNum, expectedLevel] of lineExpectedIndent) {
                    ...
                    const actualIndentStr = getActualIndentStr(line);
                    const expectedStr = indentStr(expectedLevel);

                    if (actualIndentStr !== expectedStr) {
                        const actualSpaces = indentType === "tab"
                            ? actualIndentStr.length
                            : actualIndentStr.length;
                        const expectedSpaces = indentType === "tab"
                            ? expectedLevel
                            : expectedLevel * indentSize;

                        context.report({
                            node: firstToken,
                            loc: {
                                start: { line: lineNum, column: 0 },
                                end: { line: lineNum, column: actualIndentStr.length },
                            },
                            message: `Expected indentation of ${expectedSpaces} ${indentType === "tab" ? "tab" : "space"}${expectedSpaces === 1 ? "" : "s"} but found ${actualSpaces}.`,
                        });
                    }
                }
            },
