...
const noMisleadingCharacterClassRule = {
    ...
    create(context) {
        ...
        function resolveConstIdentifierString(identifierNode) {
            ...
            const nodeStart = identifierNode.range ? identifierNode.range[0] : identifierNode.start;
            const nodeEnd = identifierNode.range ? identifierNode.range[1] : identifierNode.end;
            let scope = context.sourceCode.scopeManager.globalScope;

            for (const candidateScope of context.sourceCode.scopeManager.scopes) {
                const block = candidateScope.block;
                if (!block) {
                    continue;
                }
                ...
                const scopeContainsNode = blockStart <= nodeStart && blockEnd >= nodeEnd;
                if (!scopeContainsNode) {
                    continue;
                }

                const currentBlock = scope.block;
                const currentStart = currentBlock?.range ? currentBlock.range[0] : currentBlock?.start ?? 0;
                const currentEnd = currentBlock?.range ? currentBlock.range[1] : currentBlock?.end ?? Infinity;
                const candidateSize = blockEnd - blockStart;
                const currentSize = currentEnd - currentStart;

                if (
                    candidateSize < currentSize ||
                    (candidateSize === currentSize && candidateScope !== context.sourceCode.scopeManager.globalScope)
                ) {
