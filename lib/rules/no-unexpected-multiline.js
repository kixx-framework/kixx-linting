        ...
        return {
            ...
            Program(node) {
                ...
                for (let i = 1; i < lines.length; i++) {
                    ...
                    if (!previous || !current.startsWith("/")) {
                        continue;
                    }

                    // Heuristic for multiline regex-flag confusion:
                    // `/.../g`, `/.../s.test(...)`, etc., at the beginning of a new line.
                    const isRegexLike = /^\/[^/\n]*\/[dgimsuvy]+(?:\.test\s*\(|\b)/u.test(current);
                    if (!isRegexLike) {
                        continue;
                    }
                    ...
                    context.report({
                        node,
                        loc: {
                            start: { line: i + 1, column: lines[i].length - current.length },
                            end: { line: i + 1, column: lines[i].length - current.length + 1 },
                        },
