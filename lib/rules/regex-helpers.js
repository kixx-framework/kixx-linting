 ...
 * @returns {boolean}
 */
export function hasMisleadingCharacterClass(pattern, flags) {
        ...
        onCharacterClassEnter(ccNode) {
            ...
            function collectChars(node) {
                if (node.type === "Character") {
                    ...
                    chars.push(node.min.value);
                    chars.push(node.max.value);
            ...
            // Check for surrogate pairs in non-unicode mode
            if (!isUnicode) {
            ...
            for (let i = 0; i < chars.length - 1; i++) {
                ...
                if (
                    ...
                    (next >= 0x1DC0 && next <= 0x1DFF) ||
                    (next >= 0x20D0 && next <= 0x20FF) ||
                    (next >= 0xFE20 && next <= 0xFE2F)
                ) {
                    found = true;
                    return;
                }
            ...
            for (let i = 0; i < chars.length - 1; i++) {
                if (chars[i] >= 0x1F1E0 && chars[i] <= 0x1F1FF &&
                    chars[i + 1] >= 0x1F1E0 && chars[i + 1] <= 0x1F1FF) {
                    found = true;
                    return;
                }
            }
