...
function getCalleeInfo(node) {
    ...
    if (node.computed) {
        propertyName = getStaticPropertyName(node.property, { allowIdentifier: false });
...
function isSetterInDescriptor(node, context, isDisabledGlobal) {
    ...
    let keyName;
    if (parent.computed) {
        keyName = getStaticPropertyName(parent.key, { allowIdentifier: false });
    } else if (parent.key.type === "Identifier") {
        keyName = parent.key.name;
    } else {
        keyName = getStaticPropertyName(parent.key);
    }
