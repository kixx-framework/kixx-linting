...
const noCondAssignRule = {
    ...
    create(context) {
        ...
        return {
            ConditionalExpression(node) { checkCondition(node.test, node); },
            DoWhileStatement(node) { checkCondition(node.test, node); },
            ForStatement(node) { if (node.test) checkCondition(node.test, node); },
            IfStatement(node) { checkCondition(node.test, node); },
            WhileStatement(node) { checkCondition(node.test, node); },
        };
    },
};
