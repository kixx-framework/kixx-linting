    ...
    switch (node.type) {
        ...
        case "MemberExpression":
            return (
                ...
                getStaticPropertyName(node) === "NaN"
            );
...
function isTargetIndexMethod(node) {
    return node.type === "MemberExpression" && ["indexOf", "lastIndexOf"].includes(getStaticPropertyName(node));
}
        ...
        return {
            ...
            CallExpression(node) {
                if (enforceForIndexOf && isNaNIndex(node)) {
                    const callee = node.callee.type === "ChainExpression" ? node.callee.expression : node.callee;
                    const method = getStaticPropertyName(callee) ?? "indexOf";
