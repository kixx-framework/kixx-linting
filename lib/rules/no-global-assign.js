...
function isWritableGlobal(value) {
    return value === true || value === "writable" || value === "writeable";
}

/**
 * Parse /*global name, name:true, name:false*\/ comments and return a Map of
 * name → writable (boolean). Absent flag defaults to readonly (false).
 */
function parseGlobalComments(comments) {
    const result = new Map();
    for (const comment of comments) {
        if (comment.type !== "Block") continue;
        // Match "global ..." directive comments
        const match = /^\s*global\s+([\s\S]*)$/.exec(comment.value);
        if (!match) continue;
        ...
        for (const decl of declarations) {
            ...
            if (colonIdx >= 0) {
                ...
                flag = trimmed.slice(colonIdx + 1).trim();
            } else {
                name = trimmed;
                flag = null;
            }
            if (!name) continue;
            // Default (no flag) = readonly; ":true" = writable; ":false" = readonly
            const writable = flag === "true";
            result.set(name, writable);
        }
    }
    return result;
}
    ...
    create(context) {
        ...
        return {
            "Program:exit"(node) {
                ...
                const commentGlobals = parseGlobalComments(comments);
                ...
                function isReadonlyGlobal(name) {
                    ...
                    if (commentGlobals.has(name)) {
                        return !commentGlobals.get(name); // false = readonly
                    }
