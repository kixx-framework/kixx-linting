...
function registerPrivateDefinition(info, node) {
    if (!node.key || node.key.type !== "PrivateIdentifier") return;

    const name = node.key.name;

    if (node.type === "PropertyDefinition") {
        info.defined.set(name, createEntry(node, "field"));
        return;
    }
    ...
    if (node.kind === "get" || node.kind === "set") {
        const entry = info.defined.get(name) ?? createEntry(node, node.kind === "get" ? "getter" : "setter");
        if (!entry.node) {
            entry.node = node;
        }
        ...
        return {
            ClassBody(node) {
                ...
                for (const member of node.body) {
                    registerPrivateDefinition(info, member);
                }
            ...
            PropertyDefinition(node) {
                ...
                if (!info) return;
                registerPrivateDefinition(info, node);
            },
