...
const DEFAULT_VARS_IGNORE_PATTERN = /^_/u;
const DEFAULT_ARGS_IGNORE_PATTERN = /^_/u;
const DEFAULT_DESTRUCTURED_ARRAY_IGNORE_PATTERN = /^_/u;
 ...
 * @returns {string}
 */
function getAdditionalMessage(def, config) {
    if (!def) {
        return "";
    }

    if (def.name.parent.type === "ArrayPattern") {
        return `. Allowed unused elements of array destructuring must match ${config.destructuredArrayIgnorePattern}`;
    }

    if (def.type === "Parameter") {
        return `. Allowed unused args must match ${config.argsIgnorePattern}`;
    }
    ...
    return `. Allowed unused vars must match ${config.varsIgnorePattern}`;
}
...
function parseOptions(rawOptions) {
    ...
    return {
        varsIgnorePattern: compilePattern(
            options.varsIgnorePattern,
            DEFAULT_VARS_IGNORE_PATTERN,
        ),
        argsIgnorePattern: compilePattern(
            options.argsIgnorePattern,
            DEFAULT_ARGS_IGNORE_PATTERN,
        ),
        destructuredArrayIgnorePattern: compilePattern(
            options.destructuredArrayIgnorePattern,
            DEFAULT_DESTRUCTURED_ARRAY_IGNORE_PATTERN,
        ),
