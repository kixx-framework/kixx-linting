/**
 * lintText(sourceText, options) — synchronous entry point for the Kixx linter.
 *
 * @param {string} sourceText - JavaScript source to lint
 * @param {object} [options]
 * @param {string} [options.fileName] - File path (used in returned result and rule context)
 * @param {object} [options.rules] - Rule config object, e.g. { 'no-debugger': 'error' }
 * @param {object} [options.globals] - Global variables, e.g. { console: 'readonly' }
 * @param {'module'|'script'} [options.sourceType] - Default 'module'
 * @param {number} [options.ecmaVersion] - ECMAScript version passed to parser/scope analysis (default 2024)
 * @returns {{ filePath: string, messages: Array, errorCount: number, warningCount: number }}
 */
...
export function lintText(sourceText, options = {}) {
    ...
    const languageOptions = _normalizeLanguageOptions(options);
    const { ecmaVersion, sourceType, globals } = languageOptions;
