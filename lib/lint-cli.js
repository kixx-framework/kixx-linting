 ...
 * a target is a directory, applies literal `files` and `ignores` matching,
 * writes diagnostics to `stderr`, and returns `1` when any lint error or CLI
 * setup error occurs.
 *
 * @param {Object} [args] - CLI runtime options.
 * @param {string[]} [args.argv=[]] - Positional path arguments after the executable and script name.
...
export async function runLintCli(args) {
    ...
    try {
        const { targetPaths } = await parseCliArgv(argv, cwd);
        const configObjects = await loadConfig(cwd);
        const discoveredFiles = await collectFilesFromTargets(targetPaths);
...
/**
 * Resolves and validates optional positional lint targets.
 * @param {string[]} argv - Positional path arguments after the executable and script name.
 * @param {string} cwd - Working directory used to resolve relative targets.
 * @returns {Promise<{ targetPaths: string[] }>} Absolute target paths.
 * @throws {Error} When `argv` is not an array, or a target is missing or unsupported.
 */
export async function parseCliArgv(argv, cwd) {
    ...
    const positionals = argv.length === 0 ? ["."] : argv;
    const targetPaths = [];
...
/**
 * Collects lintable JavaScript files from one or more file or directory targets.
 * @param {string[]} targetPaths - Absolute file or directory paths.
 * @returns {Promise<string[]>} De-duplicated file paths in target argument order.
 */
export async function collectFilesFromTargets(targetPaths) {
    const targetFileGroups = await Promise.all(targetPaths.map(targetPath => collectFiles(targetPath)));
    const seenFilePaths = new Set();
    const files = [];

    for (const targetFiles of targetFileGroups) {
